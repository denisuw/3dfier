...
LABEL maintainer="Balázs Dukai <b.dukai@tudelft.nl>"
LABEL description="Builds 3dfier"

ENV LIBDIR "/opt"

# copy source code
COPY . $LIBDIR/3dfier/
#COPY --from=liblas /opt/laszip-src-2.2.0/build/ $LIBDIR/laszip/
#COPY --from=liblas /opt/libLAS-1.8.1/build/ $LIBDIR/liblas/
#COPY --from=cgal /opt/cgal-releases-CGAL-4.10/build $LIBDIR/cgal/

RUN set -x; \
    rm -rf $LIBDIR/3dfier/build; \
    cd $LIBDIR/3dfier && mkdir build && cd ./build; \
    cmake .. \
